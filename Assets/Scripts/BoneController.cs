...
namespace UnityEngine.XR.ARFoundation.Samples
{
    public class BoneController : MonoBehaviour
    {
        ...
        {
            var joints = body.joints;   //신체 관절 배열 
            if (!joints.IsCreated)
                return;
            
            for (int i = 0; i < k_NumSkeletonJoints; ++i)       
            {
                XRHumanBodyJoint joint = joints[i];             //신체 관절 
                var bone = m_BoneMapping[i];                    //프리팹 관절 
                
                if (bone != null)
                {
                    bone.transform.parent = body.transform;
                    bone.transform.localPosition = joint.anchorPose.position * body.estimatedHeightScaleFactor;
                    bone.transform.localRotation = joint.anchorPose.rotation;
                    bone.transform.position += Vector3.down * 0.03f;
                }
            }
        }
